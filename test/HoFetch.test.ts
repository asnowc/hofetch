...
test("请求与响应", async function ({ hoFetch, mockFetch }) {
  ...
  expect(new URL(request.url).search, "由默认转换器转换 params 参数").toBe(
    "?p1=9&search=abc",
  );
...
test(
  "默认情况下，如果响应不成功的状态码，应抛出异常",
  async function ({ hoFetch, mockFetch }) {
    mockFetch.mockImplementation(async () => {
      return new Response(null, { status: 404 });
    });
    await expect(hoFetch.fetch("/test"), "Response.ok 为 false 应抛出异常")
      .rejects.toThrowError(HoFetchStatusError);
    await expect(
      hoFetch.fetch("/test", { ignoreFailedStatus: true }),
      "Response.ok 为 false, 但是配置了 ignoreFailedStatus 为 true, 则认为成功",
    ).resolves.instanceof(HoResponse);
  },
);
test(
  "默认能够解析带 application/json body",
  async function ({ hoFetch, mockFetch }) {
    mockFetch.mockImplementation(async () => {
...
test(
  "未知 content-type 响应头，response.bodyData 将是 ReadableStream<Uint8Array>",
  async function ({
    hoFetch,
    mockFetch,
  }) {
    ...
    const response = await hoFetch.fetch<ReadableStream<Uint8Array>>(
      "/unknown",
      {},
    );
    expect(response.bodyData).instanceof(ReadableStream);

    const response2 = await hoFetch.fetch<ReadableStream<Uint8Array>>(
      "/custom",
      {},
    );
...
async function decodeText(stream: ReadableStream<Uint8Array>) {
  //@ts-ignore
  const textList = await Array.fromAsync(
